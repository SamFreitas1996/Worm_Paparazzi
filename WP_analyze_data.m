% WP - analyze the zstack data

function [raw_sess_data_aft, raw_sess_data_bef, raw_sess_data_integral,...
    censored_wells2,runoff_cutoff2,raw_sess_data_aft_bw,raw_sess_data_bef_bw] = ...
    WP_analyze_data(data_storage,reduce_final_noise,runoff_cutoff,censored_wells,...
    bw_analysis,final_data_export_path,full_exp_name)

% make directories for the processed data 
mkdir([data_storage '/processed_data']);
mkdir([data_storage '/raw_data/diff_imgs'])
mkdir([data_storage '/processed_data/proc_zstacks']);

% set up the data dir variable 
...
data_dir(ismember( {data_dir.name}, {'.', '..',...
    'nth_sess_activity.mat','newROIs.mat','peaks.mat','sess_reg_idx.mat','censor.mat'})) = [];  %remove . and ..
...
parfor i = 1:length(data_names)
    ...
    try
        ...
        % necessary for use with the healthspan network 
        ...
        % if binarized anaylsis (default and recommended) 
        if bw_analysis
            
            % mask and binarize the two sessions 
            ...
            temp_incompleted_data_idx = zeros(1,240);
            
            % loop through each ROI and integrate the binarized sections 
            for j = 1:240
                
                % find the specific ROI
                loop_ROI = (sess_data.thisROI==j);
                
                % if the ROI doesnt exist then its incomplete 
                if isequal(sum(loop_ROI(:)),0)
                    temp_incompleted_data_idx(j) = 1;
                end
                
                % integrate the bw sessions across the ROIs 
            ...
            % if you dont use the BW analysis
            % this isnt recommended but is avaliable 
            % same thing but weighted with 
            
            sess_data.zstack_bef = (sess_data.zstack_bef).*sess_activity_integral_mask;
            sess_data.zstack_aft = (sess_data.zstack_aft).*sess_activity_integral_mask;
...
% create the incomplete data censor 
