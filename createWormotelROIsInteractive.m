% Create WormotelROIs
function [ref_img,ROI,N_wells,figs] = createWormotelROIsInteractive(fimg,save_flag)


% error('createWormotelROIsInteractive has hardcoded parameters that need to be updated along the lines of create24WellROIsInteractive before general use');
...
% Overlay well locations
    ...
    plot(plate_x_vals,plate_y_vals,'oc','MarkerSize',28,'LineWidth',2)
    
% Draw circular ROIs
    %R = 1384-1342; % Normal radius is 42
    R = mean([
                (calib_points(2,4) - calib_points(1,4)) / 10;
                (calib_points(5,4) - calib_points(4,4)) / 10;
                (calib_points(8,4) - calib_points(7,4)) / 10;
                (calib_points(11,4) - calib_points(10,4)) / 10;
                ]);
                
                % Radius of the full squares
%     R = R*0.65; % Radius of shrunken squares
    R = R*.9;      % full fat radiususususes
    
    wellposlist = [plate_x_vals(:),plate_y_vals(:),repmat(R,[numel(plate_x_vals),1])];
    wellposlist_rect = [plate_x_vals(:),plate_y_vals(:)] - R;
    wellposlist_rect = cat(2,wellposlist_rect,repmat([2*R,2*R],[length(wellposlist_rect),1]));
    
    BW = zeros(size(ref_img),'double');
    BW=insertShape(BW,'FilledRectangle',wellposlist_rect,'Color',[255 255 255]);
    BW = mean(BW,3)>0;
...
% Sort ROIs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% sortLabelByCentroid is custom
    [Labels_Sorted,old_order,new_order] = sortLabelsByCentroid(ROI,12,20);
    ROI = Labels_Sorted;
    figs{1} = getframe(gcf);
...
if save_flag
    % Save file
        fout = 'C:\Dropbox\Autogans Supporting Files\WWAnalyzer\ROI_Reference\Reference_Rig_Wormotel240.mat';
        save(fout,'ref_img','ROI','N_wells');

    % Save image
        imwrite(ref_img,'C:\Dropbox\Autogans Supporting Files\WWAnalyzer\ROI_Reference\Reference_Rig_Wormotel240.png')
        imwrite(uint8(ROI),'C:\Dropbox\Autogans Supporting Files\WWAnalyzer\ROI_Reference\ROI_Rig_Wormotel240.png')
end
